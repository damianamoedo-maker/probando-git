...
preparar_escenario_conflicto() {
  ...
  echo "Vamos a crear un conflicto controlado entre ${def} y conflict-demo."
  echo
...
mostrar_menu() {
  ...
  imprimir_cabecera "${NOMBRE_PROGRAMA} — Práctica Git (básico)"
  ...
  echo "  (q / salir del programa - terminar)"
  echo
  if ! ejercicio_completado 0; then
    ...
    mensaje_aviso "Se recomienda seguir el orden 0→10: cada ejercicio debería construirse sobre el anterior - o casi."
  fi
}
...
configurar_git_ubuntu() {
  ...
  if es_ubuntu_o_debian; then
    mensaje_info "Se ha detectado un sistema: Ubuntu/Debian."
  else
...
principal() {
  ...
  while true; do
    ...
    case "$eleccion" in
      ...
      *) mensaje_aviso "Eso es una opción no válida. Usa 0-10, r (README) o q para salir." ;;
    esac
  done
}
